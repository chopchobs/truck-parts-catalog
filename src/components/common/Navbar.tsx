...
export default function Navbar() {
  ...
  return (
    <header className="w-full font-sans shadow-sm sticky top-0 z-50">
      {/* 1. Top Bar (แถบข้อมูลติดต่อด้านบนสุด) */}
        ...
        <div className="container mx-auto px-4 py-4 flex justify-between items-center gap-8">
          ...
          <div className="hidden md:flex flex-1 max-w-2xl relative">
            <input 
              ...
              className="w-full bg-slate-50 border border-slate-200 text-sm rounded-full py-3 pl-5 pr-12 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent transition-all"
            />
            <button className="absolute right-1 top-1 bottom-1 bg-teal-600 hover:bg-teal-700 text-white p-2 rounded-full transition-colors flex items-center justify-center w-10">
          ...
          <div className="hidden md:flex items-center gap-6 text-sm font-bold text-slate-600">
            ...
            <Link href="/products" className="hover:text-blue-600 transition-colors">หมวดหมู่สินค้า</Link>
            <Link href="/contact" className="hover:text-blue-600 transition-colors">ติดต่อเรา</Link>
            
            <button className="relative p-2 text-slate-800 hover:text-blue-600 transition-colors">
              <svg className="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M3 3h2l.4 2M7 13h10l4-8H5.4M7 13L5.4 5M7 13l-2.293 2.293c-.63.63-.184 1.707.707 1.707H17m0 0a2 2 0 100 4 2 2 0 000-4zm-8 2a2 2 0 11-4 0 2 2 0 014 0z"></path></svg>
              <span className="absolute top-0 right-0 bg-red-500 text-white text-[9px] font-black w-4 h-4 rounded-full flex items-center justify-center">
                0
              </span>
            </button>
          </div>
      ...
      {/* 3. Mobile Menu (แสดงเมื่อกดปุ่ม Hamburger) */}
      {isMobileMenuOpen && (
        <div className="md:hidden bg-white border-b border-slate-100 px-4 py-4 space-y-4 shadow-xl absolute w-full left-0">
          <div className="relative">
            <input 
              ...
              className="w-full bg-slate-50 border border-slate-200 text-sm rounded-xl py-3 pl-4 pr-10 focus:outline-none focus:ring-2 focus:ring-blue-500"
            />
